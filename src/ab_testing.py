...
class ABTest:
    ...
    def record_prediction(self, 
        ...
        with self.lock:
            if variant_name not in self.predictions:
                logger.warning("Unknown variant in A/B test",
            ...
            # Check if we've reached the minimum sample size for all variants
    ...
    def is_ready_for_analysis(self) -> bool:
        ...
        """
        return all(count >= self.config.min_sample_size 
                  for count in self.result.sample_counts.values())
        ...
        with self.lock:
            ...
            for metric_name in self.config.metrics:
                ...
                for variant_name in self.predictions:
                    ...
                    elif metric_name == "business_impact" and "impact_score" in self.metadata[variant_name][0]:
...
class ABTestingManager:
    ...
    def create_test(self, config: TestConfig) -> ABTest:
        ...
        Raises:
            TestingError: If test with the same name already exists
        """
        with self.lock:
            test_name = config.name
            
            if test_name in self.active_tests:
                error_msg = f"A/B test '{test_name}' already exists"
    ...
    def stop_test(self, test_name: str) -> Optional[TestResult]:
        """
        Stop an A/B test and store its result.
        ...
        with self.lock:
            ...
            if not test:
                logger.warning("Cannot stop test - not found",
                             test_name=test_name)
                return None
    ...
    def get_assignment(self, test_name: str, user_id: str) -> Optional[TestVariant]:
        ...
        Returns:
            Assigned test variant if test found, None otherwise
        """
        with self.lock:
            test = self.active_tests.get(test_name)
    ...
    def record_prediction(self,
                         ...
                         actual: Optional[Union[float, int, List]] = None):
        """
        Record a prediction for test analysis.
        
        Args:
            test_name: Test name
            variant_name: Variant name
            response: Prediction response
            actual: Optional actual value/label
        """
        with self.lock:
            test = self.active_tests.get(test_name)
            
            if not test:
                logger.warning("Cannot record prediction - test not found",
                             test_name=test_name)
                return
